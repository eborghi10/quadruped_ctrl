...
void ConvexMPCLocomotion::run(Quadruped<float>& _quadruped,
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    const float stance_time = gait->getCurrentStanceTime(dtMPC, i);
    ...
    float pfx_rel = seResult.vWorld[0] * (.5 + 0.0) * stance_time +  // cmpc_bonus_swing = 0.0
...
void ConvexMPCLocomotion::solveDenseMPC(
  ...
  const float cmpc_x_drag = 3.0;

  if (vxy[0] > 0.3 || vxy[0] < -0.3) {
    // x_comp_integral += cmpc_x_drag * pxy_err[0] * dtMPC / vxy[0];
    x_comp_integral += cmpc_x_drag * pz_err * dtMPC / vxy[0];
  }

  // printf("pz err: %.3f, pz int: %.3f\n", pz_err, x_comp_integral);

  const int jcqp_max_iter = 10000;
  const double jcqp_rho = 0.0000001;
  const double jcqp_sigma = 0.00000001;
  const double jcqp_alpha = 1.5;
  const double jcqp_terminate = 0.1;
  const double use_jcqp = 0.0;
