...
#include <MPC_Ctrl/GraphSearch.h>
...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt, int _iterations_between_mpc)
    ...
    , standing(horizonLength, Vec4<int>(0, 0, 0, 0), Vec4<int>(11, 11, 11, 11), "Standing")
    ...
    , walking(horizonLength, Vec4<int>(0, horizonLength/2.0, horizonLength/4.0, 3.0*horizonLength/4.0),
          Vec4<int>(3.0*horizonLength/4.0, 3.0*horizonLength/4.0, 3.0*horizonLength/4.0, 3.0*horizonLength/4.0),
          "Walking")
...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data) {
  ...
  // Expected linear velocity in body's coordinate system
  Vec3<float> v_des_robot(_x_vel_des, _y_vel_des, 0);
  // Expected linear velocity in world's coordinate system
  Vec3<float> v_des_world =
    omniMode ? v_des_robot : seResult.rBody.transpose() * v_des_robot;
  // Actual robot speed in world coordinate system
  ...
  // TODO: Plot trajectory (DRAW_DEBUG_PATH, DRAW_DEBUG_SWING)
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int *mpcTable, ControlFSMData<float> &data, bool omniMode) {
  ...
  if ((iterationCounter % iterationsBetweenMPC) == 0)
  {
    ...
    if (data.userParameters->cmpc_use_sparse > 0.5) {
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
  ...
  // printf("current position: %3.f %.3f %.3f\n", p[0], p[1], p[2]);
  ...
  if (vxy[0] > 0.3 || vxy[0] < -0.3) {
    ...
    x_comp_integral += data.userParameters->cmpc_x_drag * pz_err * dtMPC / vxy[0];
  }

  // printf("pz err: %.3f, pz int: %.3f\n", pz_err, x_comp_integral);

  update_solver_settings(data.userParameters->jcqp_max_iter, data.userParameters->jcqp_rho,
      data.userParameters->jcqp_sigma, data.userParameters->jcqp_alpha, data.userParameters->jcqp_terminate,
      data.userParameters->use_jcqp);
