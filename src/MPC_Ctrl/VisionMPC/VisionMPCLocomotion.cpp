...
template<>
void VisionMPCLocomotion::run(Quadruped<float> &_quadruped, LegController<float> &_legController,
    StateEstimatorContainer<float> &_stateEstimator, std::vector<double> gamepadCommand, int gaitType,
    const DMat<float> & height_map, const DMat<int> & idx_map) {
  (void)idx_map;
  ...
  gaitNumber = gaitType;  // data.userParameters->cmpc_gait;
  ...
  v_des_world[0] = gamepadCommand[0];
  v_des_world[1] = gamepadCommand[1];
  v_des_world[2] = 0.;
  rpy_des[2] = seResult.rpy[2];
  v_rpy_des[2] = gamepadCommand[2];
  ...
  updateMPCIfNeeded(mpcTable, _stateEstimator);

  Vec4<float> se_contactState(0,0,0,0);

  const bool use_wbc = false;
  ...
  for(int foot = 0; foot < 4; foot++)
  {
    ...
    if(swingState > 0) // foot is in swing
    {
      ...
      if(!use_wbc){
        ...
        //singularity barrier
        _legController.commands[foot].tauFeedForward[2] =
          50*(_legController.datas[foot].q(2)<.1)*_legController.datas[foot].q(2);
      }
    }
...
void VisionMPCLocomotion::updateMPCIfNeeded(
...
void VisionMPCLocomotion::solveDenseMPC(
    int *mpcTable, StateEstimatorContainer<float> &_stateEstimator) {
  auto seResult = _stateEstimator.getResult();
